...
def main():
    ...
    for m in cards:
        d = dir_of(m.group(1))
        en_page = d and os.path.isfile(os.path.join(ROOT, "en", d, "index.html"))
        if not en_page:
    ...
    print(f"en/index.html: カード {len(kept)} 件(英語版なしで除外 {len(dropped)} 件)。残った日本語 {len(left)} 箇所" + (f" 例: {left[:5]}" if left else ""))
