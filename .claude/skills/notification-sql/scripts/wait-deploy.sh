#!/usr/bin/env bash
# push 後、公開URL(motitown.com/notification/)にデプロイされるのを待つ。
# 素の curl は Cloudflare の WAF が 403 にするので、除外済みの UA を付けて取る。
...
URL="https://motitown.com/notification/$N/"
UA="MotitownOpsCheck/1.0"
TITLE="$(sed -n 's/^title:[[:space:]]*//p' "$REPO_DIR/$N/draft.md" 2>/dev/null | head -1)"
[ -n "$TITLE" ] || { echo "title を $N/draft.md から取れません" >&2; exit 2; }
start=$(date +%s)
while :; do
  body="$(curl -s -A "$UA" -H 'Cache-Control: no-cache' "$URL?t=$(date +%s)" || true)"
