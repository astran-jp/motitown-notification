...
- サムネイルは `assets/` に幅 900px・品質 75 の WebP。新規は `assets/notice-{N}.webp`。元画像は `{N}/assets/list.png`(一覧専用。あれば優先)、無ければ `{N}/assets/header.png`
- **カードはワイド表示で高さ 150px 固定**(STUDIO 由来の CSS)。サムネイルは 1000:380 以下の高さ(推奨 1000:300。カード幅 320px で 100px 以内)にする。縦長の画像(支給バナー等)をそのまま使うと記事名が隠れる。縦長のときは `add-to-list.py` が 1000:380 に中央クロップするが、文字入りの画像は切れて読めなくなるので、一覧専用の `list.png`(タイトル文字なし・1000×300)を Codex で作って置く。記事のヘッダーは変えない
