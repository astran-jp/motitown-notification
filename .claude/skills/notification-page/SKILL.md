...
公開URLは `https://motitown.com/notification/{N}/` になる。
...
- `<head>` の `<meta charset="utf-8">` の直後に `<meta name="robots" content="noindex">` を置く(全ページ共通。検索結果に出さない)
...
## 7. 英語版を作る(このスキルの中で続けて行う)

ユーザーが日本語版の表示を確認したら、そのまま `/notification-en` の手順(`.claude/skills/notification-en/SKILL.md`)を実行して `{N}/en.json`・`en/{N}/index.html`・英語バナー・英語一覧 `en/index.html` まで作り、英語版のスクリーンショットも見せる。ユーザーに別コマンドを打たせない。英語ビルドのスクリーンショットが要る画像があれば、ここでユーザーに相談する。

## 8. 次の段階へ

日本語版・英語版ともユーザーが確認したら、この段階では push しない。「/notification-sql で配信用SQLを作成し公開」を案内する(push は SQL と一緒に行う)。
